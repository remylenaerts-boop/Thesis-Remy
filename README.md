...
Required for `pool_detector.py`. The server launches it automatically, so you don't need to start it yourself.
...
The server locates ffmpeg automatically. No PATH configuration is needed on Windows if you used winget.
...
**IntelliJ IDEA Community Edition** is the easiest option, because it detects the Maven project automatically when you open the folder.
